               ...
               double SH[2], double dSHt[2], double dSHp[2])
{
  ...
  const double tmp0 = sqrt((2.0*(double)l+1.0)/(4.0*PI)*fact0/fact1);

  /* P_l^|m| */

  Associated_Legendre(l,abs_v2(m),cos(theta),ALeg);

  /* Ylm */

  const double co = cos((double)m*phi);
  const double si = sin((double)m*phi);
...
void Associated_Legendre(int l, int m, double x, double ALeg[2])
{
  /*****************************************************
   ...
   where x=cos(theta)
  ******************************************************/
  const double cut0=1.0e-15,cut1=1.0e-20;
  //double p0,p1,dP; 
  //int i,ll;
  ...
  if (m>0){
    ...
    double f = 1.0;
    const double tmp0 = sqrt((1.0 - x)*(1.0 + x));
  #pragma _NEC loop_count(4)
  ...
  if (l==m){
    ...
    const double tmp0 = sqrt(1.0-x*x);
    const double dP = (cut1<tmp0) ? ((double)l*x*p0 - (double)(l+m)*p1)/tmp0 : 0.0;
  ...
  //else{
    ...
    //Pm1 = x*(2.0*(double)m + 1.0)*Pm;
      ...
      double Pm1 = x*(2.0*(double)m + 1.0)*Pm;
      double p0 = Pm1; 
      double p1 = Pm;
      const double tmp0 = sqrt(1.0-x*x);
    ...
    /* calculate Plm, l>m+1 */
      ...
      double p0 = x*(2.0*(double)m + 1.0)*Pm;
      double p1 = Pm;
  ...
  #pragma _NEC novector
      for (int ll=m+2; ll<=l; ll++){
        const double tmp0 = (x*(2.0*(double)ll-1.0)*p0 - ((double)ll+(double)m-1.0)*p1)/(double)(ll-m);
        p1  = p0;
        p0 = tmp0;
      }
