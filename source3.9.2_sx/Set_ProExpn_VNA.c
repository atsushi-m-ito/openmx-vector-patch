...
//printf("AITUNE: MEM-size = %d * %d * %d * %d\n",30, 2*30+1, (List_YOUSO[25]+1)* List_YOUSO[24] * (2*(List_YOUSO[25]+1)+1), Num_RVNA *(2*List_YOUSO[35]+1));
//    dcomplex***** CsumNL0_spe = Allocate4D_dcomplex(30, 2*30+1, (List_YOUSO[25]+1)* List_YOUSO[24] * (2*(List_YOUSO[25]+1)+1), Num_RVNA *(2*List_YOUSO[35]+1)); /*LL, m, (L0*Mul0*M0), (L*M1) */
...
#else
#pragma omp parallel for  private(m, n,mn, sum, L ,tmp01,L1, GL, Mul1, ene, L2 ,tmp0, L3) default(shared) if(Spe_Total_NO[Cwan] > 8)
		    for (mn=0; mn< Spe_Total_NO[Cwan]* Spe_Total_NO[Hwan]; mn++){
			m= mn/Spe_Total_NO[Hwan];
			n= mn%Spe_Total_NO[Hwan];
#endif
		      sum = 0.0;
		      L = 0;
...
#else
			  tmp01[L1] += DS_VNA[0][Mc_AN][k][m][L]*DS_VNA[0][Matomnum+1][kl][n][L];
#endif
...
for (mn=0; mn< Spe_Total_NO[Cwan]* Spe_Total_NO[Hwan]; mn++){
    ...
    #else
		      tmp01[L1] += DS_VNA[0][Mc_AN][k][m][L]*DS_VNA[0][Mj_AN][kl][n][L]; 
    #endif
		      L++;
		    }
