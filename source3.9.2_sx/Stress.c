...
for (int Nc = 0; Nc<GridN_Atom[Gc_AN]; Nc++) {
	...
	void Get_dOrbitals_work2(int wan, double x, double y, double z, double *dChi, int L0, int Mul0, int rank_for_m)
	 */
	...
	const double Rmin = 10e-14;

	xyz2spherical(x, y, z, 0.0, 0.0, 0.0, S_coordinate);
	double R = S_coordinate[0];
	double Q = S_coordinate[1];
	double P = S_coordinate[2];

	if (R < Rmin) {
//x = x + Rmin;
//y = y + Rmin;
//z = z + Rmin;
xyz2spherical(x + Rmin, y + Rmin, z + Rmin, 0.0, 0.0, 0.0, S_coordinate);
R = S_coordinate[0];
Q = S_coordinate[1];
P = S_coordinate[2];
	}
...
const int m = 4;
const double rm = Spe_PAO_RV[wan][m];
...
const double x1 = rm - Spe_PAO_RV[wan][m - 1];
const double x2 = rm - Spe_PAO_RV[wan][m];
const double y1 = x1 / h2;
const double y2 = x2 / h2;
const double y12 = y1*y1;
const double y22 = y2*y2;

double dum = h1 + h2;
const double dum1 = h1 / h2 / dum;
const double dum2 = h2 / h1 / dum;
dum = h2 + h3;
const double dum3 = h2 / h3 / dum;
const double dum4 = h3 / h2 / dum;
...
const double dum = f3 - f2;
const double g1 = dum*dum1 + (f2 - f1)*dum2;
const double g2 = (f4 - f3)*dum3 + dum*dum4;

const double f = y22*(3.0*f2 + h2*g1 + (2.0*f2 + h2*g1)*y2)
	+ y12*(3.0*f3 - h2*g2 - (2.0*f3 - h2*g2)*y1);

const double df = 2.0*y2 / h2*(3.0*f2 + h2*g1 + (2.0*f2 + h2*g1)*y2)
	+ y22*(2.0*f2 + h2*g1) / h2
...
RF = a*R*R*R + b*R*R + c*R + d;
dRF = 3.0*a*R*R + 2.0*b*R + c;
...
#ifdef _NEC
int m;
#pragma _NEC loop_count(32)
#pragma _NEC unroll(32)
...
double x1 = R - Spe_PAO_RV[wan][m - 1];
double x2 = R - Spe_PAO_RV[wan][m];
double y1 = x1 / h2;
double y2 = x2 / h2;
double y12 = y1*y1;
double y22 = y2*y2;
...
{
	if (L0 == 0) {
AF[0] = 0.282094791773878;
dAFQ[0] = 0.0;
dAFP[0] = 0.0;

double dChiR = dRF * AF[0];
double dChiQ = RF * dAFQ[0];
double dChiP = RF * dAFP[0];

dChi0[Nc][i1+0][0] = -dRx*dChiR - dQx*dChiQ - dPx*dChiP;
dChi0[Nc][i1+0][1] = -dRy*dChiR - dQy*dChiQ - dPy*dChiP;
dChi0[Nc][i1+0][2] = -dRz*dChiR - dQz*dChiQ - dPz*dChiP;
...
AF[0] = dum*coP;
AF[1] = dum*siP;
AF[2] = 0.48860251190292*coQ;

dAFQ[0] = 0.48860251190292*coQ*coP;
dAFQ[1] = 0.48860251190292*coQ*siP;
dAFQ[2] = -0.48860251190292*siQ;

dAFP[0] = -0.48860251190292*siP;
dAFP[1] = 0.48860251190292*coP;
dAFP[2] = 0.0;
	
	#pragma _NEC loop_count(3)
	#pragma _NEC unroll(3)
//	#pragma _NEC novector
for (int M0 = 0; M0 <= 2 * 1; M0++) {


	double dChiR = dRF * AF[M0];
	double dChiQ = RF * dAFQ[M0];
	double dChiP = RF * dAFP[M0];

	//dChi[M0*4+0] = RF * AF[M0];
	dChi0[Nc][i1+M0][0] = -dRx*dChiR - dQx*dChiQ - dPx*dChiP;
	dChi0[Nc][i1+M0][1] = -dRy*dChiR - dQy*dChiQ - dPy*dChiP;
	dChi0[Nc][i1+M0][2] = -dRz*dChiR - dQz*dChiQ - dPz*dChiP;
...
AF[0] = 0.94617469575756*coQ*coQ - 0.31539156525252;
AF[1] = 0.54627421529604*dum1*(1.0 - 2.0*siP*siP);
AF[2] = 1.09254843059208*dum1*siP*coP;
AF[3] = dum2*coP;
AF[4] = dum2*siP;

dAFQ[0] = -1.89234939151512*siQ*coQ;
dAFQ[1] = 1.09254843059208*siQ*coQ*(1.0 - 2.0*siP*siP);
dAFQ[2] = 2.18509686118416*siQ*coQ*siP*coP;
dAFQ[3] = 1.09254843059208*(1.0 - 2.0*siQ*siQ)*coP;
dAFQ[4] = 1.09254843059208*(1.0 - 2.0*siQ*siQ)*siP;

/* RICS note 72P */

dAFP[0] = 0.0;
dAFP[1] = -2.18509686118416*siQ*siP*coP;
dAFP[2] = 1.09254843059208*siQ*(1.0 - 2.0*siP*siP);
dAFP[3] = -1.09254843059208*coQ*siP;
dAFP[4] = 1.09254843059208*coQ*coP;
/* Chi */
	#pragma _NEC loop_count(5)
	#pragma _NEC unroll(5)
//	#pragma _NEC novector
for (int M0 = 0; M0 <= 2 * 2; M0++) {
...
AF[0] = 0.373176332590116*(5.0*coQ*coQ*coQ - 3.0*coQ);
AF[1] = 0.457045799464466*coP*siQ*(5.0*coQ*coQ - 1.0);
AF[2] = 0.457045799464466*siP*siQ*(5.0*coQ*coQ - 1.0);
AF[3] = 1.44530572132028*siQ*siQ*coQ*(coP*coP - siP*siP);
AF[4] = 2.89061144264055*siQ*siQ*coQ*siP*coP;
AF[5] = 0.590043589926644*siQ*siQ*siQ*(4.0*coP*coP*coP - 3.0*coP);
AF[6] = 0.590043589926644*siQ*siQ*siQ*(3.0*siP - 4.0*siP*siP*siP);

dAFQ[0] = 0.373176332590116*siQ*(-15.0*coQ*coQ + 3.0);
dAFQ[1] = 0.457045799464466*coP*coQ*(15.0*coQ*coQ - 11.0);
dAFQ[2] = 0.457045799464466*siP*coQ*(15.0*coQ*coQ - 11.0);
dAFQ[3] = 1.44530572132028*(coP*coP - siP*siP)*siQ*(2.0*coQ*coQ - siQ*siQ);
dAFQ[4] = 2.89061144264055*coP*siP*siQ*(2.0*coQ*coQ - siQ*siQ);
dAFQ[5] = 1.770130769779932*coP*coQ*siQ*siQ*(-3.0 + 4.0*coP*coP);
dAFQ[6] = 1.770130769779932*coQ*siP*siQ*siQ*(3.0 - 4.0*siP*siP);

/* RICS note 72P */

dAFP[0] = 0.0;
dAFP[1] = 0.457045799464466*siP*(-5.0*coQ*coQ + 1.0);
dAFP[2] = 0.457045799464466*coP*(5.0*coQ*coQ - 1.0);
dAFP[3] = -5.781222885281120*coP*coQ*siP*siQ;
dAFP[4] = 2.89061144264055*coQ*siQ*(coP*coP - siP*siP);
dAFP[5] = 1.770130769779932*siP*siQ*siQ*(1.0 - 4.0*coP*coP);
dAFP[6] = 1.770130769779932*coP*siQ*siQ*(1.0 - 4.0*siP*siP);

/* Chi */
	#pragma _NEC loop_count(7)
	#pragma _NEC unroll(7)
for (int M0 = 0; M0 <= 2 * 3; M0++) {
...
//#pragma _NEC novector
for(int rank = 0; rank < rank_for_m; ++rank){
	if((mp_max - mp_min) > 1){
m = (mp_min + mp_max) / 2;
if (Spe_PAO_RV[wan][m] < R)
	mp_min = m;
...
dPz = 0.0;
	...
	#pragma _NEC loop_count(17)
	#pragma _NEC unroll(17)
	//#pragma _NEC novector
	for (int m = -L0; m <= L0; m++) {
...
double dChiR = dRF * AF[i];
double dChiQ = RF * dAFQ[i];
double dChiP = RF * dAFP[i];

//dChi[M0*4+0] = RF * AF[M0];
dChi0[Nc][i1+i][0] = -dRx*dChiR - dQx*dChiQ - dPx*dChiP;
dChi0[Nc][i1+i][1] = -dRy*dChiR - dQy*dChiQ - dPy*dChiP;
dChi0[Nc][i1+i][2] = -dRz*dChiR - dQz*dChiQ - dPz*dChiP;
                        ...
                        if (0 <= MNc)
                        {
                            if (E_Field_switch == 1)
                            {
                                ...
                                if (ProExpn_VNA == 0)
                                {
                                    ...
                                    Vpt = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[spin][MNc] + F_VNA_flag * VNA_Grid[MNc] + F_VEF_flag * VEF_Grid[MNc];
                                }
                                else
                                {

                                    Vpt = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[spin][MNc] + F_VEF_flag * VEF_Grid[MNc];
                                }
                            }
                            else
                            {
                                if (ProExpn_VNA == 0)
                                {
                                    ...
                                    Vpt = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[spin][MNc] + F_VNA_flag * VNA_Grid[MNc];
                                }
                                else
                                {
                    ...
                    if (0 <= MNc)
                    {
                        if (E_Field_switch == 1)
                        {
                            ...
                            if (ProExpn_VNA == 0)
                            {
                                ...
                                ReVpt11 = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[0][MNc] + F_VNA_flag * VNA_Grid[MNc] + F_VEF_flag * VEF_Grid[MNc];

                                ReVpt22 = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[1][MNc] + F_VNA_flag * VNA_Grid[MNc] + F_VEF_flag * VEF_Grid[MNc];
                            ...
                            else
                            {
                                ...
                                ReVpt11 = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[0][MNc] + F_VEF_flag * VEF_Grid[MNc];

                                ReVpt22 = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[1][MNc] + F_VEF_flag * VEF_Grid[MNc];
                        ...
                        else
                        {
                            ...
                            if (ProExpn_VNA == 0)
                            {
                                ...
                                ReVpt11 = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[0][MNc] + F_VNA_flag * VNA_Grid[MNc];

                                ReVpt22 = F_dVHart_flag * dVHart_Grid[MNc] + F_Vxc_flag * Vxc_Grid[1][MNc] + F_VNA_flag * VNA_Grid[MNc];
                  ...
                  for (int spin = 0; spin <= SpinP_switch; spin++) {
                      ...
                      for (int i = 0; i<NO0; i++) {
                              ...
                              for (int Nog = 0; Nog < NumOLG[Mc_AN][h_AN]; Nog++)
                              {
                                  ...
                                  for (int j=0; j<30; j++){
                                    if(j<NO1){
                                      tmp0 += Orbs_Grid_FNAN_t3[j][Nog] * DM[0][spin][Mc_AN][h_AN][i][j];
                                    }
                                  }

                                  double Vpt = Vpot_grid[spin][Nc];
                                  tmp0 *= Vpt;
                                  const double tmpx = dChi0[Nc][i][0] * tmp0;
                                  const double tmpy = dChi0[Nc][i][1] * tmp0;
                                  const double tmpz = dChi0[Nc][i][2] * tmp0;


                                  /* begin(yshiihara) */
                                  /* The minus sign is for switching the sign of dDen_Grid*/
                                  sumt0 -= dxyz[Nc][0] * tmpx;
                                  sumt1 -= dxyz[Nc][1] * tmpx;
                                  sumt2 -= dxyz[Nc][2] * tmpx;
                                  sumt3 -= dxyz[Nc][0] * tmpy;
                                  sumt4 -= dxyz[Nc][1] * tmpy;
                                  sumt5 -= dxyz[Nc][2] * tmpy;
                                  sumt6 -= dxyz[Nc][0] * tmpz;
                                  sumt7 -= dxyz[Nc][1] * tmpz;
                                  sumt8 -= dxyz[Nc][2] * tmpz;
                                  /* end(yshiihara) */
                              } /* Nog */
                      ...
                      for (int i = 0; i<NO0; i++) {
                          ...
                          for (int j = 0; j<NO1; j++) {
                              ...
                              for (int Nog = 0; Nog < NumOLG[Mc_AN][h_AN]; Nog++)
                              {
                                  ...
                                  const double tmp0 = Orbs_Grid_FNAN_t3[j][Nog] * DM[0][spin][Mc_AN][h_AN][i][j] * Vpt;
                        ...
                        for (int i = 0; i<NO0; i++) {
                            ...
                            for (int j = 0; j<NO1; j++) {
                                ...
                                for (int Nog = 0; Nog < NumOLG[Mc_AN][h_AN]; Nog++)
                                {
                                    ...
                                    const double tmp0 = Orbs_Grid[Mh_AN][Nh][j] * DM[0][spin][Mc_AN][h_AN][i][j];

                                    const double tmpx = ai_dorbs0[i][0] * tmp0;
                                    const double tmpy = ai_dorbs0[i][1] * tmp0;
                                    const double tmpz = ai_dorbs0[i][2] * tmp0;
                        ...
                        for (int i = 0; i<NO0; i++) {
                            ...
                            for (int j = 0; j<NO1; j++) {
                                ...
                                for (int Nog = 0; Nog < NumOLG[Mc_AN][h_AN]; Nog++)
                                {
                                    ...
                                    const double tmp0 = Orbs_Grid_FNAN[Mc_AN][h_AN][Nog][j] * DM[0][spin][Mc_AN][h_AN][i][j];
...
//#pragma omp barrier /* this barrier is necessary to wait clearing sumx, sumy, sumz, sumt */
 ...
 * 				**********************************************************/
    ...
    if (XC_switch == 4)
    {
        ...
        for (spin = 0; spin <= spinmax; spin++)
        {
            ...
            for (i = 0; i < 9; i++)
                Stress2[spin][i] = Stress2[spin][i] * GridVol;
          ...
          dcomplex *****H_sts)
{
  ...
  ****************************************************/
  ...
  Sumz_ = (double*)malloc(sizeof(double*)*ian*jan);
  #endif

#ifdef _NEC
	
    double ***tno0_m, ***tno0_n, **idx1, *idx2;
    tno0_m = (double***)malloc(sizeof(*tno0_m)*4+sizeof(**tno0_m)*4*ian+sizeof(double)*4*ian*24);
    idx1 = (double**)(tno0_m+4);
    idx2 = (double *)(idx1+4*ian);
    for (int i=0; i<4; i++){
      tno0_m[i] = idx1;
      for (int j=0; j<ian; j++){
        idx1[j] = idx2;
    ...
    for (int i=0; i<4*ian*24; i++) tno0_m[0][0][i] = 0.0;
    tno0_n = (double***)malloc(sizeof(*tno0_n)*4+sizeof(**tno0_n)*4*jan+sizeof(double)*4*jan*24);
    idx1 = (double**)(tno0_n+4);
    idx2 = (double *)(idx1+4*jan);
    for (int i=0; i<4; i++){
      tno0_n[i] = idx1;
      for (int j=0; j<jan; j++){
        idx1[j] = idx2;
    ...
    for (int i=0; i<4*jan*24; i++) tno0_n[0][0][i] = 0.0;
#endif
    for (k=0; k<=FNAN[Gc_AN]; k++){
    ...
    l = 0;
   double *eneValues = (double*)malloc(sizeof(double)*Spe_Num_RVPS[wakg]*7);
   for (int i=0; i<Spe_Num_RVPS[wakg]*7; i++) eneValues[i] = 0.0;
   if (Mj_AN<=Matomnum) Mj_AN2 = Mj_AN;
   else   Mj_AN2 = Matomnum + 1;
...
#pragma _NEC novector
	...
	if(l<=24)
	{
        for (m=0; m<ian; m++){
          for (l3=0; l3<l ; l3++){
            tno0_m[1][m][l3] = DS_NL1[0][1][Mc_AN][k][m][l3];
            tno0_m[2][m][l3] = DS_NL1[0][2][Mc_AN][k][m][l3];
            tno0_m[3][m][l3] = DS_NL1[0][3][Mc_AN][k][m][l3];
          }
        }
        for (n=0; n<jan; n++){
          for (l3=0; l3<l ; l3++) tno0_n[0][n][l3] = DS_NL1[0][0][Mj_AN2][kl][n][l3];
        }
      ...
      for (int x=0; x<ian*jan; x++){
        ...
        for (l3=0; l3<24; l3++){
         ene = eneValues[l3];
          Sumx_[x] += ene*tno0_m[1][m][l3]*tno0_n[0][n][l3];
          Sumy_[x] += ene*tno0_m[2][m][l3]*tno0_n[0][n][l3];
          Sumz_[x] += ene*tno0_m[3][m][l3]*tno0_n[0][n][l3];
	}
      }
    }	
	else
{
      for (int x=0; x<ian*jan; x++){
        ...
        for (l3=0; l3<l; l3++){
          ene = eneValues[l3];
          Sumx_[x] += ene*DS_NL1[0][1][Mc_AN][k][m][l3]*DS_NL1[0][0][Mj_AN2][kl][n][l3];
          Sumy_[x] += ene*DS_NL1[0][2][Mc_AN][k][m][l3]*DS_NL1[0][0][Mj_AN2][kl][n][l3];
