...
inline double NLRF_BesselF_ByIndex(int Gensi, int L, int so, double R, int m);
	  ...
	  h = (kmax - kmin)/(double)OneD_Grid;
	    ...
	    for (int L0=0; L0<=Spe_MaxL_Basis[Cwan]; L0++){
	      for (int Mul0=0; Mul0<Spe_Num_Basis[Cwan][L0]; Mul0++){
			  for (int L=1; L<=Spe_Num_RVPS[Hwan]; L++){
...
#pragma _NEC ivdep
	  for (int i=0; i<=OneD_Grid; i++){
		...
		//for (int L=1; L<=Spe_Num_RVPS[Hwan]; L++){
		  ...
		  Bessel_Pro1 = NLRF_BesselF_ByIndex(Hwan,L,so,Normk, n_i);
  ...
  ****************************************************/
  ...
  if (VPS_j_dependency[wakg]==0){
   ...
   for (L1=1; L1<=Spe_Num_RVPS[wakg]; L1++){
    L += 2*Spe_VPS_List[wakg][L1]+1;
   }
   double *eneVal = (double*)malloc(sizeof(double)*L);
   int index=0;
    for (L1=1; L1<=Spe_Num_RVPS[wakg]; L1++){
    ...
    for (m=0; m<Spe_Total_NO[Cwan]; m++){
      for (n=0; n<Spe_Total_NO[Hwan]; n++){
	      ...
	      for (L3=0; L3<L; L3++){
	        sum += eneVal[L3]*DS_NL[0][0][Mc_AN][k][m][L3]*DS_NL[0][0][Mj_AN][kl][n][L3];
	      }
...
double NLRF_BesselF_ByIndex(int Gensi, int L, int so, double R, int m){
