...
#define Criterion_Max_Step            0.10  //AITUNE version before AI3.8.1AI8 is 0.050 Bohr, default of OpenMX is 0.20//
...
void Steepest_Descent(int iter, int SD_scaling_flag)
{
  ...
  ****************************************************/
  ...
  My_Max_Force = 0.0;
  double inner_f_prev_f = 0.0; /*AITUNE*/
  double abs_f = 0.0; /*AITUNE*/
  double prev_abs_f = 0.0; /*AITUNE*/
  for (Mc_AN=1; Mc_AN<=Matomnum; Mc_AN++){
    ...
    for (j=1; j<=3; j++){
      if (atom_Fixed_XYZ[Gc_AN][j]==0){
        ...
        inner_f_prev_f += Gxyz[Gc_AN][16+j]*Gxyz[Gc_AN][13+j]; /*AITUNE*/
        prev_abs_f += Gxyz[Gc_AN][13+j]*Gxyz[Gc_AN][13+j]; /*AITUNE*/
        Gxyz[Gc_AN][13+j] = Gxyz[Gc_AN][16+j]; /*AITUNE*/
      }
    }
    abs_f += tmp0;/*AITUNE*/
    tmp0 = sqrt(tmp0); 
  ...
  ****************************************************/
  ...
  {/*AITUNE begin*/
    ...
    ff[1] = prev_abs_f;
    ff[2] = inner_f_prev_f;
    printf("AITUNE(%d): ff[0:2]=%f,%f,%f\n", myid,ff[0],ff[1],ff[2]); fflush(NULL);
    MPI_Allreduce(&ff[0], &ffsum[0], 3, MPI_DOUBLE, MPI_SUM, mpi_comm_level1);
    printf("AITUNE(%d): ffsum[0:2]=%f,%f,%f\n", myid,ffsum[0],ffsum[1],ffsum[2]); fflush(NULL);
    abs_f = sqrt(ffsum[0]);
    prev_abs_f = sqrt(ffsum[1]);
    inner_f_prev_f = ffsum[2];
  }/*AITUNE end*/
  ...
  ****************************************************/
...
#if 1 /*AITUNE begin*/
    if(abs_f * prev_abs_f > 0.0){
        const double x = inner_f_prev_f / (abs_f * prev_abs_f);
        SD_scaling *= (0.9 + 0.6 * x);//(1.0 + 0.5 * x);
        if (myid==Host_ID){
                printf("AITUNE: abs_f,prev_abs_f,inner_f_prev_f=%f,%f,%f\n", abs_f,prev_abs_f,inner_f_prev_f);
                printf("AITUNE: scaling_ratio=%lf\n", x);
        }
    }
#else /*AITUNE end*/
  ...
  SD_scaling = (( Criterion_Max_Step < Max_Force*SD_scaling ) ? Criterion_Max_Step / Max_Force : SD_scaling ); /*AITUNE*/
...
/*AITUNE    
  ...
  ****************************************************/
  ...
  for (Gc_AN=1; Gc_AN<=atomnum; Gc_AN++){
    ID = G2ID[Gc_AN];
    MPI_Bcast(&Gxyz[Gc_AN][1],  3, MPI_DOUBLE, ID, mpi_comm_level1);
    MPI_Bcast(&Gxyz[Gc_AN][14], 3, MPI_DOUBLE, ID, mpi_comm_level1);//AITUNE to use forces at previous step//
