...
for (AN=1; AN<=atomnum; AN++){
    ...
    tnoA = Spe_Total_CNO[wanA];
        for (LB_AN=0; LB_AN<=FNAN[GA_AN]; LB_AN++){
                ...
                if(maxSize<tnoA*tnoB){
                        maxSize=tnoA*tnoB;
                }
        }
}
int *iArr = (int*) malloc(sizeof(int)*maxSize);
      ...
      for (i=0; i<index; i++){
          ...
          prow = brow%np_rows;
            ...
            pcol = bcol%np_cols;
              ...
              if (SCF_iter==1 || all_knum!=1){
                Cs[(jl-1)*na_rows+il-1].r += S1[kt]*co;
                Cs[(jl-1)*na_rows+il-1].i += S1[kt]*si;
              }

              Hs[(jl-1)*na_rows+il-1].r += H1[kt]*co;
              Hs[(jl-1)*na_rows+il-1].i += H1[kt]*si;
...
ftrace_region_begin("B13");
  ...
  #else  //_NEC
   if(1 == empty_states_flag || 1 == empty_occupation_flag)
   {
    for (kloop=0; kloop<T_knum; kloop++){
      for (spin=0; spin<=SpinP_switch; spin++){
	for (l=1; l<=MaxN; l++){
